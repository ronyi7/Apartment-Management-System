			...
			<table class="table table-bordered datatable" id="table-1">
				...
				<c:forEach items="${sessionScope.search_apartments_photos }" var="i">
					<tr class="odd gradeX">
						<td>${i.photo_id }</td>
						<td>${i.file_name }</td>
						<td>${i.encrypted_name }</td>
						<td>${i.addApartmentVO.apartment_id }</td>
						
						
						<td>
						 <a onclick="return confirmDelete()" href="<%=request.getContextPath()%>/addApartmentPhotosController?flag=delete&photo_id=${i.photo_id}">DELETE</a>
						</td>
